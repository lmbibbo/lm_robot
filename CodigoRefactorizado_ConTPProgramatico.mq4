...
void ModifySLTP(int ticket, double avgPrice, double sl, double tp)
  {
   ...
   for(int retry = 0; retry < 5; retry++)
     {
      RefreshRates();
      if(OrderModify(ticket, orderPrice, sl, tp, 0, Yellow))
        {
         PrintFormat("ModifySLTP OK ticket=%d sl=%.5f tp=%.5f price=%.5f", ticket, sl, tp, orderPrice);
         return;
        }
      int err = GetLastError();
      PrintFormat("ModifySLTP retry %d/5 ticket=%d error=%d price=%.5f sl=%.5f tp=%.5f", retry+1, ticket, err, orderPrice, sl, tp);
      if(err == 130 || err == 145) { Sleep(200); }
      else { Sleep(500); if(retry >= 2) break; }
     }
   PrintFormat("ModifySLTP FAILED ticket=%d sl=%.5f tp=%.5f", ticket, sl, tp);
  }
